...
from pictureplot import plot_precision_recall_vs_iterations,plot_pr_curve,plot_precision_recall,append_metrics
...
for ann in coco_data["annotations"]:
    image_category[ann["image_id"]].append(get_category_id(ann["category_id"]))

image_category_val = defaultdict(list)
for ann in coco_data_val["annotations"]:
    image_category_val[ann["image_id"]].append(get_category_id(ann["category_id"]))
...
def evaluate_model(model, data_loader, device):
    ...
    precision_labels = precision_score(all_labels, all_preds, average=None)  # average=None 表示计算每个标签的指标
    recall_labels = recall_score(all_labels, all_preds, average=None)
...
def train_model(model,data_loader,num_epochs,optimizer,device):
    ...
    for epoch in range(num_epochs):
        ...
        append_metrics(precision, recall, f1, Hamming_Accuracy)

        precision_all.append(precision)
        recall_all.append(recall)
        f1_all.append(f1)
        Hamming_Accuracy_all.append(Hamming_Accuracy)
    ...
    plot_precision_recall_vs_iterations(precision_all,recall_all,f1_all,Hamming_Accuracy_all)
...
# test model
# model = torch.load('model_final.pth', weights_only=False)
# precision, recall, f1, Hamming_Accuracy, val_loss,hamming_loss1 = evaluate_model(model, val_loader, device)
# print(f"Validation Precision: {precision:.4f}, Recall: {recall:.4f}, F1: {f1:.4f}, Hamming Accuracy: {Hamming_Accuracy:.4f}, Val Loss: {val_loss:.4f}, hamming loss:{hamming_loss1:.4f}")